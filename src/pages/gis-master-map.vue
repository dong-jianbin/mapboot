  ...
  export default {
    ...
    mounted () {
      ...
      MapLoader().then(AMap => {
        ...
        // that.infoWindow = new window.AMap.AdvancedInfoWindow({})
        ...
        that.indexplaceSearch = new window.AMap.PlaceSearch({
          // map: that.map
        }) // 构造地点查询类
        window.AMap.event.addListener(auto, 'select', that.select)// 注册监听，当选中某条记录时会触发
        // window.AMap.event.addListener(that.MplaceSearch, 'markerClick', that.markerclick)// 注册监听，当选中某条记录时会触发
        // 搜索结果显示
        window.AMap.service(['AMap.PlaceSearch'], function () {
          that.MplaceSearch = new window.AMap.PlaceSearch({ // 构造地点查询类
            pageSize: 5, // 单页显示结果条数
            pageIndex: 1, // 页码
            city: '010', // 兴趣点城市
        ...
        let clickHandler = function (e) {
          ...
          placeSearch.searchNearBy('', cpoint, 200, function (status, result) {
            ...
            let content = '<div>' + infopoi.name + '</div><div>' + '地址：' + infopoi.address + '</br>' +
              ...
              '<div>'
            // console.log(content)
            that.infowindowmapclick = new window.AMap.AdvancedInfoWindow({
              content: content,
    ...
    methods: {
      ...
      select: function (e) {
        ...
        that.MplaceSearch.search(e.poi.name, function (status, result) {
          ...
          //   var poi = pois[i]
          //   // that.marker = []
          //   that.marker[i] = new window.AMap.Marker({
