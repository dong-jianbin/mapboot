    ...
    methods: {
      ...
      initmap () {
        ...
        MapLoader().then(AMap => {
          ...
          // 构造公交换乘类
          that.transfer = new window.AMap.Transfer(that.transOptions)
        }, e => {
      ...
      initautoinput () {
        let that = this
        // 输入提示
        let autostart = new window.AMap.Autocomplete({
          input: 'start'
        })
        let autoapproach1 = new window.AMap.Autocomplete({
          input: 'approach1'
        })
        let autoapproach2 = new window.AMap.Autocomplete({
          input: 'approach2'
        })
        let autostop = new window.AMap.Autocomplete({
          input: 'stop'
        })
        window.AMap.event.addListener(autostart, 'select', that.selectstart)// 注册监听，当选中某条记录时会触发
        window.AMap.event.addListener(autoapproach1, 'select', that.selectapproach1)// 注册监听，当选中某条记录时会触发
        window.AMap.event.addListener(autoapproach2, 'select', that.selectapproach2)// 注册监听，当选中某条记录时会触发
        window.AMap.event.addListener(autostop, 'select', that.selectstop)// 注册监听，当选中某条记录时会触发
      },
      clickinput () {
      ...
      startdriving () {
        ...
        if (!that.approach1latlng && !that.approach2latlng) {
          ...
          // 根据起终点经纬度规划驾车导航路线
          that.driving.search(new window.AMap.LngLat(that.startlatlng[0], that.startlatlng[1]), new window.AMap.LngLat(that.stoplatlng[0], that.stoplatlng[1]), function (status, result) {
          ...
          if (that.approach1latlng && that.approach2latlng) {
            ...
            // 两个途径点
            that.driving.search(new window.AMap.LngLat(that.startlatlng[0], that.startlatlng[1]), new window.AMap.LngLat(that.stoplatlng[0], that.stoplatlng[1]), {
              waypoints: [new window.AMap.LngLat(that.approach1latlng[0], that.approach1latlng[1]), new window.AMap.LngLat(that.approach2latlng[0], that.approach2latlng[1])]
            ...
            // 根据起终点经纬度规划驾车导航路线
            that.driving.search(new window.AMap.LngLat(that.startlatlng[0], that.startlatlng[1]), new window.AMap.LngLat(that.stoplatlng[0], that.stoplatlng[1]), {
              waypoints: [new window.AMap.LngLat(that.approach1latlng[0], that.approach1latlng[1])]
            }, function (status, result) {
              // result 即是对应的驾车导航信息，相关数据结构文档请参考  https://lbs.amap.com/api/javascript-api/reference/route-search#m_DrivingResult
              if (status === 'complete') {
                document.getElementById('panel').style.visibility = 'visible'// 显示导航信息
            ...
            // 根据第二个途径点规划
            that.driving.search(new window.AMap.LngLat(that.startlatlng[0], that.startlatlng[1]), new window.AMap.LngLat(that.stoplatlng[0], that.stoplatlng[1]), {
              waypoints: [new window.AMap.LngLat(that.approach2latlng[0], that.approach2latlng[1])]
            }, function (status, result) {
              // result 即是对应的驾车导航信息，相关数据结构文档请参考  https://lbs.amap.com/api/javascript-api/reference/route-search#m_DrivingResult
              if (status === 'complete') {
                document.getElementById('panel').style.visibility = 'visible'// 显示导航信息
      ...
      starttrans () {
        let that = this
        // 根据起、终点坐标查询公交换乘路线
        // that.transfer.search(new window.AMap.LngLat(116.291035, 39.907899), new window.AMap.LngLat(116.427281, 39.903719), function (status, result) {
        ...
        if (that.startlatlng != null && that.stoplatlng != null) {
          ...
          console.log('stoppoint:' + that.stoplatlng)
          // 根据起终点经纬度规划驾车导航路线
          that.transfer.search(new window.AMap.LngLat(that.startlatlng[0], that.startlatlng[1]), new window.AMap.LngLat(that.stoplatlng[0], that.stoplatlng[1]), function (status, result) {
            // result 即是对应的驾车导航信息，相关数据结构文档请参考  https://lbs.amap.com/api/javascript-api/reference/route-search#m_DrivingResult
            if (status === 'complete') {
              document.getElementById('panel').style.visibility = 'visible'// 显示导航信息
              // console.log('绘制驾车路线完成')
